...
import 'package:moje_badania/services/profile_request.dart';
...
class _LoadingProfileState extends State<LoadingProfile> {
  ...
  void startRequest(ProfileRequest profileReq) async {
    await profileReq.getProfile();
    if (profileReq.status == 200) {
      String role = await getStringValue('MB_role');
      if(role == 'PACJENT') {
        Navigator.pushNamed(context, '/profilepacjent', arguments: profileReq.ret);
      } else if(role == 'LEKARZ') {
        Navigator.pushNamed(context, '/profilelekarz', arguments: profileReq.ret);
      } else if(role == 'DIAGNOSTA') {
        Navigator.pushNamed(context, '/profilediagnosta', arguments: profileReq.ret);
      } else {
        Navigator.popUntil(context, ModalRoute.withName('/'));
      }

    } else if (profileReq.status == 403) {
      errorLogin();
    } else {
      errorConnection();
    }
